...
- **Transcription** via OpenAI Whisper API (cloud) or faster-whisper (local)
- **Translation** using DeepSeek, OpenAI, or Ollama (local)
...
```bash
# For cloud Whisper transcription and OpenAI translation
...
Provider Options:
  --whisper [cloud|local] Whisper mode for transcription (default: cloud)
...
The tool generates intermediate files that can be used to resume processing:

- `{name}.whisper.{lang}.srt` - Transcription output
- `{name}.{llm}.{lang}.srt` - Final translated subtitles
